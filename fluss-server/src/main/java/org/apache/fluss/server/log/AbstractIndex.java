...
public abstract class AbstractIndex implements Closeable {
    ...
    /** The maximum index size in bytes. */
     ...
     * (2) at loading segments from disk or truncating back to an old segment where a new log
     * segment became active; we want to reset the index size to maximum index size to avoid rolling
     * new segment.
     *
     * @param newSize new size of the index file
     * @return a boolean indicating whether the size of the memory map and the underneath file is
     *     changed or not.
     */
    ...
    public void trimToValidSize() throws IOException {
        inLock(lock, () -> resize(entrySize() * entries));
    }
    ...
    /*
     * Fluss mmaps index files into memory, and all the read / write operations of the index is through OS page cache. This
     ...
     * cache. Fluss always appends to the end of the index file, and almost all the index lookups (typically from in-sync
     * followers or consumers) are very close to the end of the index. So, the LRU cache replacement policy should work very
     * well with Fluss's index access pattern.
     *
     ...
     * steps:       |1| | | | | |3| | |4|  |5 |2/6|
     * In each page, there are hundreds log entries, corresponding to hundreds to thousands of fluss messages. When the
     ...
     *    SPARC, Power, ARM etc.).
     * 2. This number is large enough to guarantee most of the in-sync lookups are in the warm-section. With default Fluss
     ...
     * 2) make sure the warm section of low QPS table-buckets are really warm.
     */
